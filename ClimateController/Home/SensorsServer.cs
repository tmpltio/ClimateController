...
internal sealed class SensorsServer(int port)
{
    ...
    private void HandleMessage(ReadOnlyMemory<byte> buffer)
    {
        ...
        if (message.Method == Method.NotifyFullStatus)
        {
            ...
            sensor.Climate.Temperature = parameters.Temperature.Value;
            sensor.Climate.Humidity = parameters.Humidity.Value;
        }
    }
    ...
    private sealed class CompoundSensor : Sensor
    {
        ...
        {
            public override Task<float> GetTemperatureAsync(CancellationToken cancellationToken = default) =>
        ...
        {
            public override Task<float> GetHumidityAsync(CancellationToken cancellationToken = default) =>
